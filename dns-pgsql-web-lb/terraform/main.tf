###############################################################################
# Terraform - Provisionnement de l'infrastructure complete
# Projet Virtualisation M1 - Datacenter Prive
#
# Composants :
#   - dns-rec-01  : AdGuard Home (recursif + filtrage)
#   - dns-auth-01 : BIND9 (autoritaire infra.lan)
#   - pgsql-01    : PostgreSQL 17
#   - web-01/02/03 : Cluster web HA (Nginx + PHP)
#   - lb-01/02    : Load balancers HAProxy + Keepalived
###############################################################################
...
# =============================================================================
# Container LXC : dns-rec-01 (AdGuard Home - DNS recursif + filtrage)
# =============================================================================
resource "proxmox_virtual_environment_container" "dns_rec_01" {
  description   = "DNS recursif AdGuard Home avec filtrage publicitaire (VLAN 20 CORE)"
  node_name     = var.dns_node # pve-02
  vm_id         = 201
  tags          = ["dns", "recursive", "adguard", "vlan20", "infra"]
  start_on_boot = true
  unprivileged  = true
  ...
  initialization {
    ...
    # IP que les clients utilisent comme DNS
    ip_config {
      ipv4 {
    ...
    # DNS au boot (avant qu'AdGuard soit installe)
    dns {
      domain  = "infra.lan"
...
# =============================================================================
# Container LXC : dns-auth-01 (BIND9 - DNS autoritaire pour infra.lan)
# =============================================================================
resource "proxmox_virtual_environment_container" "dns_auth_01" {
  description   = "DNS autoritaire BIND9 pour la zone infra.lan (VLAN 20 CORE)"
  node_name     = var.dns_node # pve-02
  vm_id         = 202
  tags          = ["dns", "authoritative", "bind9", "vlan20", "infra"]
  start_on_boot = true
  unprivileged  = true

  initialization {
    hostname = "dns-auth-01"
    ...
    # Au boot, on utilise le DNS recursif (qui sera deploye juste avant)
    dns {
      domain  = "infra.lan"
...
resource "proxmox_virtual_environment_container" "pgsql_01" {
  ...
  depends_on = [
    proxmox_virtual_environment_container.dns_rec_01,
    proxmox_virtual_environment_container.dns_auth_01
  ]
}
